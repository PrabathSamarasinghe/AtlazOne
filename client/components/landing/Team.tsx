...
interface TeamProps {
  preloadedData?: TeamMember[];
}

export default function Team({ preloadedData }: TeamProps) {
  const [team, setTeam] = useState<TeamMember[]>([]);
  const [loading, setLoading] = useState(!preloadedData);

  useEffect(() => {
    if (preloadedData) {
      setTeam(preloadedData);
      setLoading(false);
      console.log("✅ Team using preloaded data:", preloadedData.length);
      return;
    }
